# Site copy

Use this as the editable copy sheet for the portfolio. Copy changes back into the site files after editing.

## Metadata

**Browser title:** Polly Davidson - Brand Strategist & Marketing Leader at GitHub

**Search description:** Portfolio for Polly Davidson, a senior brand strategist and marketing leader at GitHub working across brand, campaigns, content, social and experiences.

## Shared navigation

- Polly Davidson
- Projects
- Photos
- About
- Contact

## Homepage

### Hero

**Eyebrow:** Brand strategist / marketing leader / GitHub

**Title:** I make complicated things interesting

**Intro line:** Hi, I'm Polly

**Body:** I'm a brand strategist and marketing leader. I'm a big fan of words, creative experiments, data, and really understanding the audience. 

### Selected work

**Eyebrow:** Selected work

**Title:** Brand stories, launches and moments

#### GitHub Universe

**Eyebrow:** Flagship work

**Title:** GitHub Universe

**Description:** GitHub Universe is GitHub's flagship developer event, and I've worked on it for five years running, helping attract 3.7K attendees across brand, social, content, community and ticket sales.


#### The GitHub Shop

**Eyebrow:** Brand commerce

**Title:** The GitHub Shop

**Description:** The shop is loved by developers. It's a true brand play: creating swag that reflects their interests but also creating a playful site that ties back to developers with its collection names, fidget spinners and hidden easter eggs. I work closely with the shop team to launch their new collections, get campaigns featured in swag drops, and for influencer gifting. 



#### GitHub for Beginners

**Eyebrow:** Audience understanding

**Title:** GitHub for Beginners

**Description:** A YouTube series that turns intimidating developer tooling into useful, friendly lessons for people just getting started, hosted by Kedasha Kerr. 

**Tags:** Content strategy, Education, Video

#### The GitHub Podcast

**Eyebrow:** Developer stories

**Title:** The GitHub Podcast

**Description:** The GitHub Podcast is a show dedicated to the topics, trends, stories and culture in and around the developer community on GitHub. Designed to bring a more behind the scenes view of GitHub and showcasing how we support open source maintainers. 

#### Wunderlist

**Eyebrow:** Launch storytelling

**Title:** Wunderlist | Microsoft To Do

**Description:** Visual storytelling series designed to move users over from Wunderlist to Microsoft To Do. 

**Tags:** Brand, Product story, Visual identity

#### Video storytelling

**Eyebrow:** Video storytelling

**Title:** Complex stories, made watchable

##### Log4J

**Description:** Creating a campaign around the story of Log4J so that audiences have a full understanding of the dangers of security vulnerabilities.

##### Anders Hejlsberg

**Description:** Making sure the story of Anders Hejlsberg, creator of Typescript, C# and Turbo Pascal had a fully built out campaign in order to maximise views. 

### About

**Eyebrow:** About me

**Title:** Strategy, but make it interesting

**Paragraph 1:** I've spent the last 15+ years working in marketing and brand, mostly in developer marketing, but also in TV, banking, and consumer productivity apps, across smaller startups and larger corporates. These days I'm at GitHub, where I work collaboratively across brand, campaigns, social content and community activations. 

**Paragraph 2:** I like big ideas, useful data, good writing and work that doesn't take itself too seriously.

**What I do:**

- Brand and campaign strategy
- Developer marketing
- Content and social
- Event go-to-market

**How I think:**

- Understand your audience
- Use data without losing the essence of your brand
- Make it fun

### Personal project

**Eyebrow:** Personal project

**Title:** Days of Deutsch

**Description:** A little passion project helping people learn German, one day at a time, with a more playful kind of storytelling.

### Contact

**Eyebrow:** Contact

**Main line:** Want to talk about brand or campaign strategy? Let's connect.

**Social links:** GitHub, LinkedIn

### Footer

**Text:** Made by Polly Davidson

## Blog page

**Eyebrow:** Blog

### Draft post 1

**Label:** Draft idea

**Title:** What makes technical stories worth reading?

**Description:** A place for thoughts on brand, developer storytelling, campaigns, content and the work behind making complicated things feel clear.

### Draft post 2

**Label:** Draft idea

**Title:** Notes from building brand moments

**Description:** A future home for reflections on events, launches, creative strategy and the difference between making noise and making meaning.

## Photos page

**Work heading:** Work

**Play heading:** Play

## Image alt text

### Homepage images

- About portrait: Polly Davidson
- GitHub Universe: GitHub Universe event signage
- The GitHub Shop: GitHub Shop campaign image with summer-themed GitHub merchandise
- GitHub for Beginners: GitHub for Beginners video set with presenter and GitHub props
- The GitHub Podcast: GitHub Podcast channel artwork with Mona and Hubot characters
- Wunderlist: Wunderlist visual with pink background and orange abstract shapes
- Days of Deutsch: Franzbroetchen pastry with a handwritten Days of Deutsch label

### Photos: Work

- Child sketching GitHub characters in a notebook
- GitHub for Beginners mug and drawing materials
- GitHub for Beginners hoodie
- Person posing inside a large GitHub cat sculpture at Universe
- GitHub Universe collaboration wall with attendees writing notes
- Colourful GitHub Universe arch installation
- GitHub Universe event signage
- GitHub Shop merchandise display at an event
- Furby-inspired GitHub Universe display
- GitHub event badge with checklist
- GitHub Shop event moment
- GitHub Universe wall detail
- GitHub Universe outdoor moment
- GitHub for Beginners printed cards

### Photos: Play

- Garden table and chairs with a bright inflatable flamingo
- Sunlit mountain village built into rocky cliffs
- Decorative lanterns hanging in a warm shop window
- Oman flags hanging above a shopfront
- Child walking through an art gallery
- Large sunlit courtyard with stairs and a person in blue
- Apartment balconies and flags on a tree-lined street
- Fresh coconut seen through wooden chair slats
- Heron against a dark wall with its shadow
- People sitting in front of stormy sea windows
- Wet cobbled London street at dusk
- Silhouetted person with an umbrella in misty rain
- Restaurant window with wine bottles and a turn off phone sign
- Colourful collection of toys on a shelf
- Sunlit wall seen through a dark archway
- Coney Island boardwalk and amusement park signs
- People playing volleyball in a snowy park
- Dogs running along a quiet beach with a yellow boat
- Dark restaurant window with glasses on a table
- Mountain reflected in a bright blue lake
- Snowy mountains across a lake
- Colourful woven chair on a wooden balcony
- Person relaxing in a dark room with a work hard and be nice poster

## Link destinations

| Link text / item | Destination |
| --- | --- |
| Polly Davidson logo/name | `/` |
| Projects nav | `/#work` from other pages, `#work` on homepage |
| Photos nav | `/photos` |
| About nav | `/#philosophy` from other pages, `#philosophy` on homepage |
| Contact nav | `/#contact` from other pages, `#contact` on homepage |
| GitHub Universe | `https://githubuniverse.com` |
| The GitHub Shop | `https://thegithubshop.com` |
| GitHub for Beginners | `https://www.youtube.com/playlist?list=PL0lo9MOBetEFcp4SCWinBdpml9B2U25-f` |
| The GitHub Podcast | `https://www.youtube.com/playlist?list=PL0lo9MOBetEHmIDpT8KA-qt7Ebb2GhesV` |
| Wunderlist | `https://youtube.com/playlist?list=PLbJ3LHaydIC1XzL1mM-Dcr1wdF2sg8q0V&si=-vpgayczJqT3S5yU` |
| Log4J | `https://www.youtube.com/watch?v=t74ClffSUW0&t=11s` |
| Anders Hejlsberg | `https://www.youtube.com/watch?v=uMqx8NNT4xY&t=1s` |
| Days of Deutsch | `https://www.instagram.com/daysofdeutsch` |
| Contact GitHub | `https://github.com/pollyday` |
| Contact LinkedIn | `https://www.linkedin.com/in/polly-davidson/` |
| Footer Polly Davidson | `https://github.com/pollyday` |
